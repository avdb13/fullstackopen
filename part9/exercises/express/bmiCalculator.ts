...
// const _parseArgs = (args: Array<string>): Bmi => {
...
export const calculator = (mass: number, height: number): string => {
  const bmi = mass / Math.pow(height / 100, 2);
  const result = categories.find((c) => c.range[0] < bmi && bmi < c.range[1])!;
