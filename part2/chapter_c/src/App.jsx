...
const Footer = () => {
  ...
  return (
    <div style={footerStyle}>
      <br />
      <em>Note app, department of bad engineering, university of dummies 2023</em>
    </div>
  )
}
...
const App = () => {
  ...
  const addNote = (event) => {
    event.preventDefault()
      ...
      create(note).then(note => setNotes([...notes, note]))
      setNewNote('')
  }
  ...
  const toggleImportance = (id) => {
    ...
    update(changed, id)
      .then(updated => setNotes(notes.map(n => n.id === id ? updated : n)))
      .catch(error => {
        setErrorMessage(
    ...
    <div>
      ...
      <ul>
        {notesToShow.map(note => {
          return <Note toggleImportance={() => toggleImportance(note.id)} note={note} key={note.id} />
        })}
      </ul>
